    ...
    def listen_serial(self):
        ...
        if not self.recording:
            if len(self.buffer) == 512:
                window = np.array(self.buffer) - np.mean(self.buffer)
                if verify_ppg_waveform(window):
                    self.label.configure(text="Good signal detected. Recording...", fg="blue", bg="white")
                    self.recording = True
                    self.countdown = 30
            ...
            self.label.configure(text=f"Recording in progress: {self.countdown} sec", fg="blue", bg="white")
